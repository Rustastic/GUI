...
impl NodeDetails {
    ...
    pub fn render(&mut self, state: &mut GUIState, ctx: &egui::Context) {
        ...
        for (id, instance) in &state.nodes {
            if instance.selected {
                println!("Node {} selected", instance.id);
    ...
    fn render_node_window(&self, state: &mut GUIState, instance: &mut NodeGUI, ctx: &egui::Context) {
    ...
    fn get_window_title(&self, instance: &NodeGUI) -> String {
        match instance.node_type {
            NodeType::Server => {
                format!("{:?}{:?} {}", instance.server_type.unwrap(), instance.node_type, instance.id)
            }
            NodeType::Client => {
                format!("{:?}::{:?} {}", instance.client_type.unwrap(), instance.node_type, instance.id)
            }
            NodeType::Drone => {
    ...
    fn render_interactive_controls(&self, state: &mut GUIState, ui: &mut egui::Ui, instance: &mut NodeGUI) {
    ...
    fn render_remove_sender_dropdown(&self, ui: &mut egui::Ui, instance: &mut NodeGUI) {
        egui::ComboBox::from_label("Select Sender to remove:")
            .selected_text("None")
            .show_ui(ui, |ui| {
                let mut options: Vec<String> = instance.neighbor.iter()
                    .map(|n| n.to_string())
                    .collect();
    ...
    fn render_add_sender_dropdown(&self, state: &GUIState, ui: &mut egui::Ui, instance: &mut NodeGUI) {
        egui::ComboBox::from_label("Select Sender to add:")
            .selected_text("None")
            .show_ui(ui, |ui| {
                let mut options: Vec<String> = state.edges.keys()
            ...
            egui::ComboBox::from_label("Select Client:")
                ...
                .show_ui(ui, |ui| {
                    if let Some(client_list) = &instance.chat_params.client_list_value {
                        let mut options: Vec<String> = client_list.iter()
    ...
    fn render_register_to_dropdown(&self, ui: &mut egui::Ui, instance: &mut NodeGUI) {
        ...
        egui::ComboBox::from_label("Select Server to register to:")
            .selected_text("None")
            .show_ui(ui, |ui| {
                // This would iterate through communication servers
                // let options = get_communication_servers(state);
                // Implementation would be similar to other dropdowns
            });
    }
    
    fn render_media_controls(&self, state: &GUIState, ui: &mut egui::Ui, instance: &mut NodeGUI) {
        if instance.media_params.ask_for_file_list {
            self.render_file_server_dropdown(ui, instance);
        }
        
        if instance.media_params.server_value.is_some() 
            && state.file_list.contains_key(&instance.media_params.server_value.unwrap()) 
            && instance.media_params.get_file {
            self.render_file_selection(state, ui, instance);
        }
    }
    
    fn render_file_server_dropdown(&self, ui: &mut egui::Ui, instance: &mut NodeGUI) {
        egui::ComboBox::from_label("Select Server to get List:")
            .selected_text("None")
            .show_ui(ui, |ui| {
                // This would need access to text content servers from state
                // Similar implementation to other server dropdowns
            });
    }
                    ...
                    if let Some(file_list) = state.file_list.get(&server_id) {
                        ...
                        for option in options {
                            if ui.selectable_label(false, &option).clicked() {
                                ...
                                instance.command = Some(GUICommands::GetFile(instance.id, server_id, option));
